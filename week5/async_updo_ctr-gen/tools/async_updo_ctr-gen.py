...
srcDir = genDir + "src/"
flowDir = genDir + "flow/"
designDir = genDir + "designs/src/async_updo_ctr-gen/"
simDir = genDir + "simulations/"
commonDir = genDir + "../../common/"
platformDir = genDir + "../../common/platforms/" + args.platform + "/"
objDir = flowDir + "objects/" + args.platform + "/async_updo_ctr-gen/"
...
# with open(srcDir + "one_Bit_ADC.v", "r") as rf:
#     filedata = rf.read()
...
# with open(srcDir + "asynchronous_up_down_Inst.v", "r") as rf:
#     filedata = rf.read()
#     filedata = re.sub("module\s*(\w+)\s*\n", "module " + designName + "\n", filedata)
# with open(srcDir + "asynchronous_up_down_Inst.v", "w") as wf:
#     wf.write(filedata)

with open(flowDir + "design/sky130hd/async/config.mk", "r") as rf:
    filedata = rf.read()
    filedata = re.sub(
        "export DESIGN_NAME\s*=\s*(\w+)", "export DESIGN_NAME = " + designName, filedata
    )
with open(flowDir + "design/sky130hd/async/config.mk", "w") as wf:
    wf.write(filedata)

shutil.copyfile(
    srcDir + "one_Bit_ADC.v", flowDir + "design/src/async/one_Bit_ADC.v"
)
shutil.copyfile(
    srcDir + "Ring_Osc_Analog.v", flowDir + "design/src/async/Ring_Osc_Analog.v"
)
# shutil.copyfile(
#    srcDir + "vanshika_tff.v", flowDir + "design/src/async/TEMP_AUTO_def.v"
# )
shutil.copyfile(
    srcDir + "asynchronous_up_down_Inst.v", flowDir + "design/src/async/" + designName + ".v"
)
shutil.copyfile(srcDir + "vanshika_tff.v", flowDir + "design/src/async/vanshika_tff" + ".v")
...
#  subprocess.run(["ls", "-l", flowDir, "results/", args.platform, "/async"])

shutil.copyfile(
    flowDir + "results/" + args.platform + "/async/6_final.gds",
    outputDir + "/" + designName + ".gds",
)
shutil.copyfile(
    flowDir + "results/" + args.platform + "/async/6_final.def",
    outputDir + "/" + designName + ".def",
)
shutil.copyfile(
    flowDir + "results/" + args.platform + "/async/6_final.v",
    outputDir + "/" + designName + ".v",
)
shutil.copyfile(
    flowDir + "results/" + args.platform + "/async/6_1_fill.sdc",
...
shutil.copyfile(
    flowDir + "reports/" + args.platform + "/async/6_final_drc.rpt",
    outputDir + "/6_final_drc.rpt",
)
shutil.copyfile(
    flowDir + "reports/" + args.platform + "/async/6_final_lvs.rpt",
